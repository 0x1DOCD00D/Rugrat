...
public class ClassGenerator 
{
	...
	private boolean implementRunnable = false;
	...
	private HashMap<String,String>          signatureReturnTypeMap = new HashMap<String,String>();
	...
	private LinkedList<String>              calledClassList = new LinkedList<String>();
	...
	public ClassGenerator(String fileName, 
			int loc, 
			ClassGenerator superClass, boolean implementRunnable) 
	{
		super();
			...
			HashSet<String> preGeneratedClasses)
	{
		...
		//append import statements for SQL
		if(ProgGenUtil.useQueries)
		{
			appendImportStatements();
		}
		
		//append import statements for Storage Access
		appendImportStatementsForStorageAccess();
	...
	private void appendImportStatementsForStorageAccess()
	{
		program += "import java.io.BufferedWriter;\n";
		program += "import java.io.FileNotFoundException;\n";
		program += "import java.io.FileWriter;\n";
		program += "import java.io.File;\n";
		program += "import java.io.IOException;\n";
		program += "import java.io.FileReader;\n";
		program += "import java.io.InputStreamReader;\n";
		program += "import java.net.URL;\n";
...
//	public Method getMethodFromSignature(MethodSignature sig){
//		ArrayList<Method> methodlist = new ArrayList<Method>() ;
//		Method m1 = null  ;
//		methodlist = this.getMethodList() ;
//		for(Method m : methodlist){
//			if(m.getMethodSignature() == sig){
//				m1 = m ;
//				break ; 
//			}
//		}
...
//	}
	...
	//private LinkedList<String>   calledClassList = new LinkedList<String>();
	public void setCalledClassList(String calledClassName){
		
		this.calledClassList.add(calledClassName) ;
	}
	
	public LinkedList<String> getCalledClassList(){
		return this.calledClassList ;
	}
	
//	public void setSignatureReturnTypeMap(String key,String value){ // key will be te signature, value will be the return type value
//		signatureReturnTypeMap.put(key, value) ;
//	}
	
	public String getSignatureReturnTypeMap(String key){
		return signatureReturnTypeMap.get(key) ;
	}
	...
	{
		String runMethodSignature = "\npublic void run(){\n";
		String trySignature = "      try{\n";
		String catchSignature = "} catch (InterruptedException e) {\n e.printStackTrace();\n}\n";
		String methodBody = "";
		program += "\npublic static void main(String args[]){\n";
		methodBody += this.fileName + " obj = new " + this.fileName +"();\n";
		...
		if (!implementRunnable) {
			program += methodBody;
		}
		else {
			program += methodBody;
			program += runMethodSignature;
			program += trySignature ;
			program += methodBody;
			program += catchSignature ;
		}
	}
	...
	private void generateMethodSignatures(ArrayList<ClassGenerator> classList) 
	{
		for (int i = 0; i<numberOfMethods; i++) 
		{
			if(this.superClass != null)
			{
				...
				if(choiceOverride == 0) // if choiceOr == 0 the method goes to select an overridden method
				{
					MethodSignature overridenMethod = overRiddenMethod(classList, locPerMethod);
					if(overridenMethod != null)
					{
						methodSignatures.add(overridenMethod);
						signatureReturnTypeMap.put(overridenMethod.toString(),overridenMethod.getReturnType().toString() ) ; // key= method signature, value=return Type
						continue;
					}
					//else fall through and generate normal class method
				}
			}
			...
			signatureReturnTypeMap.put(method.getMethodSignature().toString(),method.getReturnType().toString() ) ; // key= method signature, value=return Type
	...
	private void appendClassName() 
	{
		...
		else if (implementRunnable) {
			program += " implements Runnable ";
		}
	...
	public boolean implementsThread() {
